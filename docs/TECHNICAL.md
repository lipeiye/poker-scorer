...
> 德扑积分器 — 完整架构与实现参考  
> 配套入口：[README.md](../README.md) · 架构与模式：[ARCHITECTURE_PATTERNS.md](./ARCHITECTURE_PATTERNS.md)  
...
| 测试 | Vitest + `@cloudflare/vitest-pool-workers`；纯结算规划测试 |
| 验证 / 部署 | GitHub Actions 自动 typecheck + test；`wrangler deploy` 手动生产部署 |
...
│   ├── pot-settlement.ts  # 主池/边池只读规划与精确 payout 预览
│   ├── player-rules.ts    # 可行动 / 争夺底池等纯谓词（Specification）
...
- `expiresAt` / `createdAt`
- `sidePots?` — 最近一次结算明细，`refund=true` 表示退还而非赢池；结算后短暂存在，下一手清空
- `lastWinnerIds` — 庆祝弹窗用

`PublicGameState` 另下发 `roundComplete`、`expiresAt`、`serverVersion`。前端用 `roundComplete` 决定是否展示「下一轮」，不再维护完整后端谓词副本。
...
| `previewEndHand` | 只读调用结算规划，向请求者返回精确 payout，不落盘 |
| `endHand` | `handleEndHand(tiers, winnerIds)` |
| `updateSettings` | 仅 waiting + **房主** 改盲注 |
| `rebuy` | 仅 waiting；给他人补码需 **房主** |
| `removePlayer` | 仅 waiting + **房主** 移除**离线**玩家 |
| `endHand` | 仅 showdown + **房主** |
| `sync` | 不改状态，只回推**请求者**（切前台补状态） |
| `ping` | `pong`（不落盘） |
...
结算:     handlePreviewEndHand, handleEndHand, awardPotsByTiers,
          awardToSoloSurvivor, checkSoloSurvivor；纯规划在 pot-settlement.ts
位次:     assignPositions, nextActiveIndex, nextDifferentActiveIndex
通信:     broadcast, publicState, sendError, sendNoticeToAll, playerIdFor
设置:     handleSettings
```
...
- **raise**：  
  - 最小增量 = `lastRaiseSize`（开街 = bigBlind；完整加注后更新）。  
  - 筹码不够 → all-in；若推高了 `currentBet` 且加注幅度 ≥ `lastRaiseSize` 才 `resetActedFlags` 并提升 min（不足额 all-in **不完整重开**）。  
  - 非 all-in：**无条件** `raiseAmount >= lastRaiseSize`。  
...
#### 边池结算 `planPotsByTiers` / `awardPotsByTiers(tiers)`

- `tiers[0]` = 第 1 名（可并列），`tiers[1]` = 第 2 名…  
- 兼容旧客户端：只发 `winnerIds` 时包装成单层 `[[...]]`。  
- 校验：id 存在、`!isFolded`、不跨档重复。  
- `src/pot-settlement.ts` 先以纯函数只读规划；预览与真实结算共用同一结果，避免算法分叉。
...
- 未跟注/超额投入层标记 `refund=true`，仍保留在 `sidePots`，但不进入 `lastWinnerIds`、不触发赢家庆祝。

#### 盲注 `postBlinds`

- 单挑：dealer = SB；多人：SB = dealer 下家（`nextActiveIndex`）  
- BB = SB 下家；强制 **SB ≠ BB**（`nextDifferentActiveIndex` 兜底）  
- 实付 `min(blind, chips)`；`currentBet = max(sbAmt, bbAmt)`（跟注对齐有效最高注；最小加注看 `lastRaiseSize`）
...
- **TTL 到期**：硬销毁——关所有 WS、`RoomRegistry.remove`、`storage.deleteAll`、标记 `expired`  
- **北京 04:00 且 waiting**：soft reset——清 pot/hand/sidePots 等进度，**保留**玩家、筹码、host、口令与房号；不 remove registry  
- `expired` 后旧 WebSocket 的排队消息/close 回调不再落盘，避免 `deleteAll` 后把房间状态写回。
...
PWA：`manifest.webmanifest` + `sw.js`（壳缓存 `pk-shell-v12`；API/WS 不拦截）。
静态资源 query `?v=12` 做 cache bust。

### 5.2 模块

| 模块 | 职责 |
|------|------|
| `app.js` | init、消息路由（含 notice/preview）、`onState`、操作栏/大厅事件委托、创建加入房间、胜者庆祝去重、SW 注册 |
| `socket.js` | 单例 WS；join 时带 playerId/deviceId；心跳 60s / pong 超时 240s；指数退避重连 max 30s；`visibilitychange` 发 `sync`+`ping` 或 `reconnectNow` |
| `render.js` | lobby/game/actionBar；`isMyTurn`；摊牌 `selectedTiers` 与 payout 预览；离线/过期时间文案 |
| `actions.js` | fold 二次确认、raise 步进、all-in、previewEndHand→endHand、rebuy/remove |
...
- 全押按钮直接 `raise(chips)`  
- 摊牌：点玩家进当前档 →「下一档」→ 请求权威预览 → 核对后确认结算；可返回修改。两人桌隐藏「下一档」，单击选择唯一胜者。
- 修改盲注与给他人补码会二次确认；大厅展示房主提示、过期时间与 serverVersion。
...
| type | 主要字段 | 说明 |
|------|----------|------|
| `join` | name, playerId?, deviceId?, password? | 加入/重连；设了口令时新玩家必填 |
| `leave` | | 主动离开 → 坐出 |
| `action` | action, amount? | fold/check/call/raise |
| `startHand` | | 开始新手 |
| `nextRound` | | 进下一街 |
| `previewEndHand` | tiers? 或 winnerIds? | 只读结算预览，不改变筹码/round |
...
| type | 字段 |
|------|------|
| `state` | `state: PublicGameState`（每连接 `yourPlayerId` / `youAreHost` 不同） |
| `error` | `message` |
| `notice` | `message`（成功/业务提示，不标记为错误） |
| `preview` | `preview: { total, pots, payouts }`；payout 标明 win/refund |
| `pong` | |
...
- 最小加注/开池下注增量：**≥ lastRaiseSize**（开街 = bigBlind；完整加注后更新）  
- 不足额 all-in 不 `resetActedFlags`、不提升 `lastRaiseSize`：已行动者只需补跟  
- 无 straddle
...
游戏判决尽量由后端下发结果；仍有展示映射时，改协议后需同步前端。

| 逻辑 | 后端 | 前端 |
|------|------|------|
| 本轮结束 | `game-room.ts` `isRoundComplete` | 只读取 `state.roundComplete`（旧实例部署切换期有私有 fallback） |
...
文件：`test/game-room.test.ts`、`test/pot-settlement.test.ts`
...
| 结算规划 | 纯函数无副作用、未排满拒绝、单人层退还、精确 payout |
| 发布前高风险回归 | 单挑 flop BB 首动、不足额 all-in 不重开、预览只读、退还不庆祝 |
| 并发重连 | SB≠BB、重连不误离线 |
...
```bash
npm run dev       # 本地
npm run check     # typecheck + test + dry-run
npm run deploy    # 先打印 cache/version 检查清单，再生产部署；记录 Current Version ID
npm run tail      # 日志
```

后端无热更新；DO 状态跨版本保留。

GitHub push / PR 会由 `.github/workflows/ci.yml` 自动执行 typecheck 与全量测试。生产仍为显式手动部署；发布前更新 `SERVER_VERSION`、静态资源 `?v=` 和 SW `CACHE`。
...
3. 轻量 host（首个 deviceId）→ 敏感写操作需房主；行动仍全员。  
4. 全量 broadcast → 状态小、调试简单；`sync` 仅回推请求者。  
5. 每日 04:00 **soft reset** + 7 天 TTL **硬销毁** → 同房码可长期用，仍可控存储。  
6. 音效移除 → 系统会把页面当媒体播放源抢耳机。  
7. 可选入桌口令 → 房号泄露时的轻量防护（明文存 DO，熟人局可接受）。
...
|----|------|
| 无 straddle | — |
| Host 不自动转移 | 房主设备永不来则敏感操作需旧房「无 host 放行」或重建 |
| 口令明文 | 熟人局；非安全边界产品 |
| 前端 raise input | HTML 默认 min=0，运行时由 render 设为 minRaise；后端仍是最终校验 |
| 模式演进 | 见 [ARCHITECTURE_PATTERNS.md](./ARCHITECTURE_PATTERNS.md) |

历史上审计与任务卡（含 soft reset / host / lastRaiseSize / 口令）已在 2026-07 落地。以**当前代码**为准。
...
| `isRoundComplete` / `advanceTurn` | 验证 `PublicGameState.roundComplete` 与前端按钮行为 |
...
3. `src/game-room.ts` + `src/pot-settlement.ts` — `handleStartHand` → `postBlinds` → `setFirstToAct` → `handleAction` → plan/apply 结算
