...
onConn((connState) => {
  ...
  // 进入连接中态视为已离开连接：下次 connected 才提示「已恢复」。
  if (connState === 'connecting' || connState === 'offline') wasDisconnected = true;
  if (connState === 'connected' && wasDisconnected && state) {
...
function leaveTable() {
  ...
  state = null;
  wasDisconnected = false; // 离开房间重置，下次进入不算「断线重连」
